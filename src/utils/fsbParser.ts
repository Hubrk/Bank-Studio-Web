 ...
 * FSB5 解析器（独立项目裁剪版）。
 *
 * 只做纯 JS 的字节解析：从任意文件（含 RIFF .bank / FMOD Studio .bank 容器）里定位 FSB5，
 * 提取每个子音频的元数据（名称 / 声道 / 采样率 / 时长 / 编码 hint）。
 * 实际解码由主线程 FMOD WASM 完成（见 fsbDecode.ts）。
 *
...
export interface FsbBankParseResult {
  /** 从容器里切出的 FSB5 字节（self-contained，解码 / 重打包基于它） */
  fsbBytes: Uint8Array;
  /** FSB5 magic 在原始文件中的起始偏移（容器写回时用） */
  fsbOffset: number;
...
export const parseFsbBank = (fileBytes: Uint8Array, knownOffset?: number | null): FsbBankParseResult | null => {
  ...
  return { fsbBytes: bytes, fsbOffset: offset, samples };
};